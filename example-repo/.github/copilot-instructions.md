<!-- gene2-figma-mcp v0.3.7 — do not remove this line -->
...
## 0 · Version check (run on first prompt of every session)

This file was generated by **gene2-figma-mcp v0.3.7**.

1. Call `bridge_read_config` — check `_gene2Version` in the response
2. If the config version is **newer** than `0.3.7` (the version in this file):
   - Tell the user: "Your gene2-figma-mcp tool is v{configVersion} but your instructions/skills are from v0.3.7. Updating now…"
   - Run: `npx gene2-figma-mcp init --only config,skills --force`
   - This regenerates copilot-instructions.md and all skill files to match the installed version
3. If versions match — proceed normally

This check ensures instructions and skills stay in sync with the installed tool version.
...
8. **Capture-first for DS pages.** Always use the capture workflow (showcase → capture → post-process) for creating DS component pages. Direct bridge creation is a last resort — only when capture is explicitly unavailable or user requests it.
9. **Local images only in showcase.** Never use external image URLs in `figma/showcase/` — they break during Figma capture. Download to `figma/showcase/public/` or use inline SVG data URIs.
10. **Real components in `src/components/`, specs in `figma/components/`.** Never put business logic or developer-owned components inside `figma/`. Showcase pages import real components from `src/` via the `@` alias.

---

## 10 · Skills (auto-loaded when relevant)
